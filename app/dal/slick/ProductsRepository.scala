...
class ProductsRepository @Inject()(dbConfigProvider: DatabaseConfigProvider) extends dal.ProductsRepository {
  ...
  def delete(code: String): Future[Int] = db.run(Products.filter(_.code === code).delete)

  def updateByCode(code: String, product: Product): Future[Int] = db.run(Products.filter(_.code === code).update(product))
